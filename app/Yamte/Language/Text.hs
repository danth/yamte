...
parseWord = do
  text <- many1 notSpace <|> many1 space
  return $ Node
    (SyntaxConstruct { _render = const $ W.str text
...
parseLine = Node
  (SyntaxConstruct { _render = \words -> case words of
                                           [] -> W.vLimit 1 $ W.fill ' '
                                           _ -> W.hBox words
                   , _stringify = concat
                   })
...
parseDocument = Node
  (SyntaxConstruct { _render = W.vBox, _stringify = unlines })
  <$> many parseLine
