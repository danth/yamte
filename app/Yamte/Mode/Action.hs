...
actions
  = [ Action (ModifiedKey (KChar 'e') []) "Switch to input mode"
    ...
    , Action (ModifiedKey (KChar 'w') [ MCtrl ]) "Move to the start of the file"
      moveTop
    , Action (ModifiedKey (KChar 's') [ MCtrl ]) "Move to the end of the file"
      moveBottom
    , Action (ModifiedKey (KChar 'a') [ MCtrl ]) "Move to the start of the line"
      moveHome
    , Action (ModifiedKey KHome []) "Move to the start of the line" moveHome
    , Action (ModifiedKey (KChar 'd') [ MCtrl ]) "Move to the end of the line"
      moveEnd
