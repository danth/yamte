...
module Data.Tree.Cursor ( TreeCursor, toTree, toCursor, moveUp, moveDown, moveLeft, moveRight ) where
...
data TreeAbove a = TreeAbove { _parent :: Maybe (TreeAbove a)
                             , _self :: a
                             , _lefts :: [Tree a]
                             , _rights :: [Tree a]
                             }

data TreeCursor a = TreeCursor { _above :: Maybe (TreeAbove a)
                               , _target :: a
                               , _below :: [Tree a]
                               }
...
moveDown cursor = do
  ...
  return $
    cursor & above ?~ TreeAbove { _parent = cursor ^. above
                                , _self = cursor ^. target
                                , _lefts = []
...
moveUp cursor = do
  above' <- cursor ^. above
  return $ cursor & above .~ (above' ^. parent)
                  & target .~ (above' ^. self)
...
moveLeft cursor = do
  ...
  let above'' = above' & lefts .~ init lefts'
                       & rights %~ (cursorToNode cursor :)
  return $ cursor & above ?~ above''
...
moveRight cursor = do
  ...
  let above'' = above' & lefts %~ (cursorToNode cursor :)
                       & rights .~ tail rights'
  return $ cursor & above ?~ above''
